...
- **Fixed a crash that could close the app at startup.** Lotus tints itself from the colours in the current track's artwork. When that colour extraction came back empty — which it can — the app shut down instead of quietly falling back to its normal colours. It now falls back.
