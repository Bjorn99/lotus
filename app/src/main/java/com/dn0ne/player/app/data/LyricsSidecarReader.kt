...
class LyricsSidecarReader(private val contentResolver: ContentResolver) {
    ...
    fun readSidecarLyrics(
        ...
        return cursor.use { c ->
            val docId = findMatchingDocumentId(c.asDocumentRows(), target) ?: return@use null
    ...
    companion object {
        // Scans a sequence of (documentId, displayName, mimeType) rows
        // for the first non-directory row whose display name matches
        // [target] (case-insensitive). Returns the matching document ID,
        // or null if nothing matches. Kept separate from readSidecarLyrics
        // so tests can exercise the row-filtering logic without needing
        // Robolectric to fake DocumentsContract statics.
        internal fun findMatchingDocumentId(
            ...
            return rows.firstOrNull { row ->
                row.mimeType != DocumentsContract.Document.MIME_TYPE_DIR &&
                    ...
                    row.displayName.equals(target, ignoreCase = true) &&
                    row.documentId != null
            }?.documentId
        }
    }
